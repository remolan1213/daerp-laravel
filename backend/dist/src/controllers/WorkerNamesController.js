...
const createWorkerName = async (req, res) => {
    try {
        const workerName = await AppDataSource.getRepository(WorkerNames).save(req.body);
...
const updateWorkerName = async (req, res) => {
    try {
        const result = await AppDataSource.getRepository(WorkerNames).update(req.params.id, req.body);
...
const deleteWorkerName = async (req, res) => {
    try {
        const result = await AppDataSource.getRepository(WorkerNames).delete(req.params.id);
        if (result.affected === 0) {
            return res.status(404).json({ error: "Worker Name not found" });
        }
...
export { getAllWorkerNames, createWorkerName, getWorkerNameById, updateWorkerName, deleteWorkerName, };
